...
# from leptonai.photon import Photon, StaticFiles
# from leptonai.photon.types import to_bool
# from leptonai.api.v0.workspace import WorkspaceInfoLocalRecord  # Updated import
# from leptonai.util import tool
...
# GOOGLE_SEARCH_CX="45faba3870edd4362"
# GOOGLE_SEARCH_API_KEY="AIzaSyAh9SiRlRaIoFu92lePDUK9k-3F2cs1Gvw"
# LEPTON_WORKSPACE_TOKEN="aVyDdkfRRpHin4SZH2G85eLxWtOmxaay"
# BACKEND="GOOGLE"
...
# # Hardcoded API and configuration values
# OPENAI_API_KEY = "sk-proj-q51oEU49FNTlB4BestwNT3BlbkFJxoarDAv4Y802pKsnKHiT"
# GOOGLE_SEARCH_CX = "45faba3870edd4362"
# GOOGLE_SEARCH_API_KEY = "AIzaSyAh9SiRlRaIoFu92lePDUK9k-3F2cs1Gvw"
# LEPTON_WORKSPACE_TOKEN = "aVyDdkfRRpHin4SZH2G85eLxWtOmxaay"
# BACKEND = "GOOGLE"
...
# KV_NAME = "search-with-lepton"
# LEPTON_API_TOKEN = "aVyDdkfRRpHin4SZH2G85eLxWtOmxaay"
...
# # Specify the default timeout for the search engine.
# DEFAULT_SEARCH_ENGINE_TIMEOUT = 5

# # If the user did not provide a query, we will use this default query.
# _default_query = "Who said 'live long and prosper'?"

# # This is the most important part of the rag model. It gives instructions
# # to the model on how to generate the answer.
# _rag_query_text = """
# You are a large language AI assistant built by Lepton AI. You are given a user question, and please write clean, concise and accurate answer to the question. You will be given a set of related contexts to the question, each starting with a reference number like [[citation:x]], where x is a number. Please use the context and cite the context at the end of each sentence if applicable.
...
# # This is the prompt that asks the model to generate related questions.
# _more_questions_prompt = """
# You are a helpful assistant that helps the user to ask related questions, based on user's original question and the related contexts. Please identify worthwhile topics that can be follow-ups, and write questions no longer than 20 words each. Please make sure that specifics, like events, names, locations, are included in follow up questions so they can be asked standalone. For example, if the original question asks about "the Manhattan project", in the follow up question, do not just say "the project", but use the full name "the Manhattan project". Your related questions must be in the same language as the original question.
...
#         except AttributeError:
#             thread_local.client = openai.OpenAI(
#                 base_url=f"https://{LLM_MODEL}.lepton.run/api/v1/",
...
#             )
#         else:
#             raise RuntimeError("Backend must be GOOGLE.")
...
#         logger.info("Creating KV. May take a while for the first time.")
#         self.kv = KV(
#             KV_NAME, create_if_not_exists=True, error_if_exists=False
#         )
#         self.should_do_related_questions = to_bool(os.getenv("RELATED_QUESTIONS", "true"))
...
#                 tools=[{
#                     "type": "function",
#                     "function": tool.get_tools_spec(lambda questions: []),
#                 }],
#                 max_tokens=512,
#             )
#             related = response.choices[0].message.tool_calls[0].function.arguments
...
#     @staticmethod
#     def _get_response_from_llm(
#         query: str, contexts: List[dict], model: str
#     ) -> httpx.AsyncIterator[dict]:
#         """
...
#                     {
#                         "role": "system",
#                         "content": _rag_query_text.format(
#                             context="\n\n".join([c["snippet"] for c in contexts])
#                         ),
#                     },
#                     {
...
#                 tools=[{
#                     "type": "function",
#                     "function": tool.get_tools_spec(lambda answers: []),
#                 }],
#                 max_tokens=1024,
#                 stream=True,
#             )
...
#         except Exception as e:
#             logger.error(
#                 f"Encountered error while generating LLM response: {e}\n{traceback.format_exc()}"
#             )
...
#     ) -> StreamingResponse:
#         """
#         Handles the search query and returns a streaming response.
#         """
#         llm_response = self._get_response_from_llm(query, contexts, self.model)
#         related_questions_future = None
#         if self.should_do_related_questions:
#             related_questions_future = self.executor.submit(
#                 self.get_related_questions, query, contexts
#             )
#         return StreamingResponse(
...
#     async def _get_related_questions(
#         self, query: str, contexts: List[dict]
#     ) -> List[str]:
#         """
#         Generates related questions based on the provided query and contexts.
#         """
#         try:
#             return self.get_related_questions(query, contexts)
#         except Exception as e:
#             logger.error(
#                 f"Encountered error while generating related questions: {e}\n{traceback.format_exc()}"
#             )
#             return []

#     @staticmethod
#     def search(
#         query: str, kv: KV, search_function, max_concurrent_searches: int = 1
#     ) -> List[dict]:
#         """
...
#         ) as executor:
#             future_to_query = {
#                 executor.submit(search_function, query): query
#             }
#             for future in concurrent.futures.as_completed(future_to_query):
#                 try:
#                     contexts += future.result()
#                 except Exception as e:
#                     logger.error(f"Encountered error while searching: {e}")
#                     raise HTTPException(status_code=500, detail="Internal Server Error")
#         return contexts

#     def get_response(self, query: str) -> StreamingResponse:
#         """
#         Gets the response based on the user query.
#         """
#         contexts = self.search(query, self.kv, self.search_function)
#         return self._get(query, contexts)

#     def get_related_questions(self, query: str) -> List[str]:
#         """
#         Gets related questions based on the user query.
#         """
#         contexts = self.search(query, self.kv, self.search_function)
#         return self._get_related_questions(query, contexts)

# if __name__ == "__main__":
#     import uvicorn
#     uvicorn.run("search_with_lepton:app", host="0.0.0.0", port=8000, log_level="info")
...
# from leptonai.photon import Photon, StaticFiles
# from leptonai.photon.types import to_bool
# from leptonai.api.v1.workspace_record import WorkspaceRecord  # Corrected import
# from leptonai.util import tool
...
#                         "name": title,
#                         "url": url,
#                         "snippet": json_content["answer_box"]
#                         .get("organic_result")
#                         .get("snippet", ""),
#                     }
#                 )
...
# def extract_contexts_from_references(references):
#     """
#     Given a list of references, extract the contexts.
#     """
#     return "\n\n".join(
#         [f"[citation:{i+1}] {ref['snippet']}\nURL: {ref['url']}" for i, ref in enumerate(references)]
#     )
...
#                 "https://search-api.lepton.run/",
#                 token=os.environ.get("LEPTON_WORKSPACE_TOKEN")
#                 or WorkspaceRecord.get_current_workspace_token(),  # Replaced WorkspaceInfoLocalRecord
#                 stream=True,
#                 timeout=httpx.Timeout(connect=10, read=120, write=120, pool=10),
#             )
...
#     def search_contexts(self, query: str, engine: str):
#         """
#         Search the contexts given a search query and the search engine.
#         """
#         if engine == "bing":
#             subscription_key = os.getenv("BING_SUBSCRIPTION_KEY")
#             if not subscription_key:
#                 raise HTTPException(500, "Missing BING_SUBSCRIPTION_KEY")
#             return search_with_bing(query, subscription_key)

#         if engine == "google":
#             subscription_key = os.getenv("GOOGLE_API_KEY")
#             cx = os.getenv("GOOGLE_CX")
#             if not subscription_key or not cx:
#                 raise HTTPException(500, "Missing GOOGLE_API_KEY or GOOGLE_CX")
#             return search_with_google(query, subscription_key, cx)

#         if engine == "serper":
#             subscription_key = os.getenv("SERPER_API_KEY")
#             if not subscription_key:
#                 raise HTTPException(500, "Missing SERPER_API_KEY")
#             return search_with_serper(query, subscription_key)

#         if engine == "searchapi":
#             subscription_key = os.getenv("SEARCHAPI_API_KEY")
#             if not subscription_key:
#                 raise HTTPException(500, "Missing SEARCHAPI_API_KEY")
#             return search_with_searchapi(query, subscription_key)

#         raise HTTPException(500, "Unsupported search engine")

#     async def rag(self, query: str, context_count: Optional[int] = None):
#         """
#         Handles a RAG (Retrieval-Augmented Generation) request.
#         """
#         context_count = context_count or REFERENCE_COUNT

#         try:
#             search_engine = os.getenv("SEARCH_ENGINE", "bing")
#             contexts = self.search_contexts(query, search_engine)

#             if len(contexts) == 0:
#                 return "Sorry, no contexts were found."

#             context_text = extract_contexts_from_references(contexts)

#             model_response = await self.local_client().generate(
#                 prompt=_rag_query_text.format(context=context_text),
#                 max_tokens=context_count * 100,
#                 temperature=0.7,
#             )

#             if not model_response.ok:
#                 logger.error(f"{model_response.status_code} {model_response.text}")
#                 raise HTTPException(500, "Error generating response from the model.")

#             response_text = model_response.json()["choices"][0]["text"]
...
#     async def more_questions(self, query: str):
#         """
#         Generates follow-up questions based on the original query and contexts.
#         """
#         try:
#             search_engine = os.getenv("SEARCH_ENGINE", "bing")
#             contexts = self.search_contexts(query, search_engine)
...
#             model_response = await self.local_client().generate(
#                 prompt=_more_questions_prompt.format(context=context_text),
#                 max_tokens=64,
#                 temperature=0.7,
#             )
...
#             if not model_response.ok:
#                 logger.error(f"{model_response.status_code} {model_response.text}")
#                 raise HTTPException(500, "Error generating follow-up questions.")

#             questions = model_response.json()["choices"][0]["text"]
...
# app = FastAPI()  # Instantiate FastAPI here
# # Define your routes below
# # app.mount("/static", StaticFiles(directory=os.path.join("web", "src", "app")), name="static")
# @app.get("/")
# def read_root():
#     return {"Hello": "World"}
# if __name__ == "__main__":
...
#             )
#         else:
#             raise RuntimeError("Backend must be LEPTON or GOOGLE.")
#         self.model = os.environ["LLM_MODEL"]
#         self.executor = concurrent.futures.ThreadPoolExecutor(
#             max_workers=self.handler_max_concurrency * 2
#         )
...
#         all_yielded = ""
#         try:
#             yield from self._raw_stream_response(
#                 contexts, llm_response, related_questions_future
#             )
#             all_yielded = "".join([chunk for chunk in self._raw_stream_response(
#                 contexts, llm_response, related_questions_future
#             )])
#             if search_uuid:
#                 self.kv[search_uuid] = all_yielded
#                 logger.info(f"Storing search result with UUID {search_uuid}.")
#         except Exception as e:
#             logger.error(f"Error while streaming response: {e}")

#     def query_search_engine(self, query, search_uuid):
#         """
#         Query the search engine and return contexts.
#         """
#         contexts = self.search_function(query)
#         if contexts:
#             logger.debug(f"Context obtained: {contexts}")
#         else:
#             logger.warning(f"No context obtained for query '{query}'")
#         return contexts

#     def do_inference(self, query, contexts, search_uuid):
#         """
#         Performs inference to generate answer and related questions.
#         """
#         # Generate LLM response.
#         try:
#             llm_response = self.local_client().chat.completions.stream(
#                 model=self.model,
#                 messages=[
#                     {
#                         "role": "system",
...
#         # Stream the response and upload to KV.
#         return self.stream_and_upload_to_kv(
#             contexts, llm_response, related_questions_future, search_uuid
#         )

#     def get(self, query: str = _default_query):
#         """
#         Gets the answer from the RAG model.
#         """
#         contexts = self.query_search_engine(query, None)
#         return StreamingResponse(
#             self.do_inference(query, contexts, None),
...
# import os
# import json
# from search_with_lepton import RAG  # Import the RAG class
...
class RAG(Photon):
    ...
    def init(self):
        ...
        Initializes photon configs.
        """
        self.backend = os.environ.get("BACKEND", "LEPTON").upper()
        if self.backend == "LEPTON":
            # Initialization for LEPTON
            self.leptonsearch_client = Client(
                "https://search-api.lepton.run/",
                token=os.environ.get("LEPTON_WORKSPACE_TOKEN"),
                stream=True,
            ...
            self.search_function = None  # No search function for LEPTON
        elif self.backend == "GOOGLE":
            # Initialization for GOOGLE
            ...
            raise RuntimeError("Backend must be LEPTON or GOOGLE.")
        self.model = os.environ.get("LLM_MODEL", "mixtral-8x7b")
        self.executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=self.handler_max_concurrency * 2
        )
        logger.info("Creating KV. May take a while for the first time.")
        self.kv = KV(
            os.environ.get("KV_NAME", "search-with-lepton"), create_if_not_exists=True, error_if_exists=False
        )
        self.should_do_related_questions = to_bool(os.environ.get("RELATED_QUESTIONS", "true"))
    ...
    def query_search_engine(self, query, search_uuid):
        ...
        if not self.search_function:
            logger.error("Search function is not defined. Ensure backend is correctly set.")
            return []
        contexts = self.search_function(query)
        if contexts:
            logger.debug(f"Context obtained: {contexts}")
...
# Instantiate and initialize the RAG class
rag_instance = RAG()
rag_instance.init()  # Ensure init is called to set up search_function and other attributes
